...
namespace Torch.Managers.PatchManager
{
    ...
    public class PatchManager : Manager
    {
        ...
        internal static void AddPatchShim(Type type)
        {
            ...
            var method = type.GetMethod("Patch", BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Static);
            if (method == null)
            {
        ...
        /// <summary>
        ///     Creates a new <see cref="PatchContext" /> used for tracking changes.  A call to <see cref="Commit" /> will apply
        ///     the patches.
        /// </summary>
        ...
        public PatchContext AcquireContext()
        {
            ...
            lock (_contexts)
            {
                if (!_contexts.TryGetValue(assembly, out var localContexts))
        ...
        /// <summary>
        ///     Frees the given context, and unregister all patches from it.  A call to <see cref="Commit" /> will apply the
        ///     unpatching operation.
        /// </summary>
        ...
        /// <summary>
        ///     Frees all contexts owned by the given assembly.  A call to <see cref="Commit" /> will apply the unpatching
        ///     operation.
        /// </summary>
        ...
        internal int FreeAllContexts(Assembly assembly, Action<PatchContext> callback = null)
        {
            ...
            var count = localContexts.Count;
            foreach (var k in localContexts)
            {
                callback?.Invoke(k);
                k.RemoveAll();
            }
        ...
        private static void DoCommit(DecoratedMethod method)
        {
            ...
            method.Commit();
            var value = Interlocked.Increment(ref _finishedPatchCount);
        ...
        internal static void CommitInternal()
        {
            lock (_rewritePatterns)
            {
                ...
                foreach (DecoratedMethod m in _rewritePatterns.Values.Where(x => x.PrintMsil))
                    DoCommit(m);
#else
                foreach (var m in _rewritePatterns.Values)
                    DoCommit(m);
#endif
        ...
        public override void Detach()
        {
            lock (_contexts)
            {
                foreach (var set in _contexts.Values)
